...
//Use a Map Instead of POJO
    ...
    // students api gives response in array hence above steps doesn't work
    RestAssured.baseURI = "http://localhost:3000/students";

    List<Students> students = RestAssured.given().queryParam("id", 1).when().get().then().extract().body().jsonPath()
